...
const columns = computed(() => {
  ...
  if (props.fit === 'fluid') return count <= 1 ? 1 : count <= 4 ? 2 : 3
  return count <= 1 ? 1 : count <= 2 ? 2 : count === 4 ? 2 : 3
})
...
<template>
  ...
  <div
    ...
    :class="[`nutmeg-member-grid--${fit}`, `nutmeg-member-grid--columns-${columns}`]"
  >
    <article v-for="(member, index) in members" :key="memberKey(member, index)" class="nutmeg-member">
      <img
...
<style scoped>
.nutmeg-member-grid { display: grid; gap: .85rem; grid-template-columns: repeat(var(--member-columns), minmax(0, 1fr)); }
...
.nutmeg-member { align-items: center; background: var(--nutmeg-card-background); border: 1px solid var(--nutmeg-card-border); border-radius: .75rem; display: flex; gap: .8rem; min-width: 0; padding: .8rem .9rem; }
...
.nutmeg-member-grid--fluid .nutmeg-member { gap: .55rem; padding: .55rem .65rem; }
...
.nutmeg-member-grid--fluid li { font-size: .55rem; }
