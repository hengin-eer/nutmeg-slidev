...
| `brand-gradient` | 黄30%・橙60%・濃橙90%のradial gradientと、半透明の白いガラス面を使う。 |
...
`NutmegQuote` は従来の単独propsに加えて、最大4件の`items`配列を受け取れます。4件の場合は2列×2行で表示し、既存の`quote`・`source`・`role`・`avatar`指定も引き続き利用できます。

```md
<NutmegQuote :items="[
  { quote: '声の本文', source: '利用者 A', role: '所属・立場' },
  { quote: '声の本文', source: '利用者 B', role: '所属・立場' }
]" />
```
